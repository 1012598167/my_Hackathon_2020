...
def gettimename(str):
    import time,datetime
    time1=datetime.datetime.strptime(str,"%Y-%m-%d %H:%M:%S")
    otherStyleTime2 = time.strftime("%Y%m%d", time1)
    return otherStyleTime2
class BaiduSpiderSpider(scrapy.Spider):
        ...
        while (END_DATE-date).days>0:
            path =r'https://www.baidu.com/s?ie=utf-8&f=8&rsv_bp=1&tn=baidu&wd=%E5%BA%B7%E5%A4%8D%E8%80%85\
                &oq=%E5%BA%B7%E5%A4%8D%E8%80%85&rsv_pq=b4c550ab003217aa&rsv_t=e5ed%2Fkfl5kKNDlIefALe9j%2Bx\
                    ...
                    +gettimestamp(date)+r'%2C'+gettimestamp(date)+r'%7Cstftype%3D2&tfflag=1'
    ...
    def parse(self, response):
        ...
        res_path = r'baiduspider/results'
        f_name = res_path + '/' + filename+ '.json'  # 要导入结果的文件名
        fw = open(f_name, 'w', encoding='utf-8')  # 覆盖写
        fw.write('1111111111\n')
        fields = response.xpath('.//div[@class="result c-container"]')
        for i in fields:
            title=fields.xpath('./h3/a//text()').extract_first().replace('\n', '')
            link=fields.xpath('./h3/a//@href').extract_first()
            new_dict={title:link}
